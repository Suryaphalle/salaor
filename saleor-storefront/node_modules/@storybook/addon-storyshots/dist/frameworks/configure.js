...
var getPreviewFile = function (configDir) {
    var preview = path_1.default.join(configDir, 'preview.js');
    var previewTS = path_1.default.join(configDir, 'preview.ts');
    var config = path_1.default.join(configDir, 'config.js');
    var configTS = path_1.default.join(configDir, 'config.ts');
    if (isFile(previewTS)) {
        return previewTS;
    }
    if (isFile(preview)) {
...
var getMainFile = function (configDir) {
    var main = path_1.default.join(configDir, 'main.js');
    var mainTS = path_1.default.join(configDir, 'main.ts');
    if (isFile(mainTS)) {
        return mainTS;
    }
    if (isFile(main)) {
...
function getConfigPathParts(input) {
    ...
    if (fs_1.default.lstatSync(configDir).isDirectory()) {
        ...
        var preview = getPreviewFile(configDir);
        var main = getMainFile(configDir);
        if (preview) {
